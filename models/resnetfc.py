...
class ResnetBlockFC(nn.Module):
    ...
    def __init__(self, size_in, size_out=None, size_h=None, beta=0.0, active = nn.ReLU()):
        super().__init__()
        # Attributes
        if size_out is None:
            size_out = size_in
...
class ResnetFC(nn.Module):             # Notice: add activation function as an argument!
    def __init__(
        self,
        d_in,
        d_out=2,
        ...
        :param beta softplus beta, 100 is reasonable; if <=0 uses the given activations instead
        """
        super().__init__()
        if d_in > 0:
        ...
        self.blocks = nn.ModuleList(
            [ResnetBlockFC(d_hidden, beta=beta, active=self.activation) for i in range(n_blocks)]
        )
            ...
            for blkid in range(self.n_blocks):
                ...
                if blkid == self.combine_layer:
                    x = combine_interleaved(
                        x, combine_inner_dims, self.combine_type    # change device
                    )  # [4, 3072, 512]
