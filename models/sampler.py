...
def get_rays_ortho(H, W, c2w, size_h, size_w):
    ...
    # # Rotate ray directions from camera frame to the world frame
    rays_d = (
        -c2w[:3, 2].view(1, 1, 3).expand(W, H, -1)
    )  # direction to center in world coordinates
    ...
    rays_o = torch.stack([(i - W * 0.5), -(j - H * 0.5), torch.zeros_like(i)], -1)
...
class ImgToPatch(object):
    ...
    def __call__(self, img):
        ...
        for img_i in img:
            ...
            _, selected_idcs, pixels_i = self.ray_sampler(
                H=self.hwf[0], W=self.hwf[1], focal=self.hwf[2], pose=pose
            )
...
class FlexGridRaySampler(RaySampler):
    ...
    def sample_rays(self, H, W):
        ...
        if self.scale_anneal > 0:
            ...
            min_scale = max(
                self.min_scale, self.max_scale * exp(-k_iter * self.scale_anneal)
            )
